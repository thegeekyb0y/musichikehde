...
export const Header = () => {
  ...
  return (
    <div className=" bg-stone-600/65 border-2 border-color3 h-12 p-2 mt-4 lg:mx-26 flex gap-2 mx-4 fixed top-0 left-0 right-0 z-50 backdrop-blur-md rounded-xl text-amber-50">
      ...
      <div className="gap-5 ml-auto items-center hidden md:flex">
        <button className="hover:opacity-60 cursor-pointer ">About Us</button>
        <button className="hover:opacity-60 cursor-pointer">Services</button>
        <button className="cursor-pointer hover:opacity-60">Pricing</button>

        <button className="relative overflow-hidden border border-green-100 bg-white/20 px-3 mr-auto cursor-pointer text-white shadow-2xl transition-all before:absolute before:bottom-0 before:left-0 before:top-0 before:z-0 before:h-full before:w-0 before:bg-green-500 before:transition-all before:duration-500 hover:text-white hover:shadow-green-500 hover:before:left-0 hover:before:w-full text-left rounded-md py-1">
      ...
      <div
        className={`absolute top-16 left-0 right-0  bg-green-500/75 p-4 flex flex-col gap-4 rounded-2xl md:hidden transition-all duration-300 ease-in-out transform ${
          isMobileMenuOpen
